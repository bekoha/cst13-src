...
/*		else if ( !Q_stricmp( argv[i], CMDLINEOPTION_NOVCONFIG ) )
		...
		{
			Warning("CSTBSP: Unknown option \"%s\"\n\n", argv[i]);
	...
	if (i != argc - 1)
	{
		...
		Warning(	
			"usage  : cstbsp [options...] mapfile\n"
			"example: cstbsp -onlyents c:\\hl2\\hl2\\maps\\test\n"
			"\n"
			...
			"                    line options).\n"
			"\n"
			"  -onlyents   : This option causes CSTBSP only import the entities from the .vmf\n"
		...
		if ( verbose )
		{
			Warning(
				...
				"  -threads     : Control the number of threads CSTBSP uses (defaults to the # of\n"
				...
				"  -noopt       : By default, CSTBSP removes the 'outer shell' of the map, which\n"
				...
				"  -micro <#>   : CSTBSP will warn when brushes are output with a volume less\n"
				...
				"  -block # #      : Control the grid size mins that CSTBSP chops the level on.\n"
				"  -blocks # # # # : Enter the mins and maxs for the grid size CSTBSP uses.\n"
	...
	//CST 1.1 - amckern - amckern@yahoo.com
	// delete all files
	sprintf (path, "%s.prt", source);
	remove (path);
	sprintf (path, "%s.lin", source);
	remove (path);
	sprintf (path, "%s.log", source);
	remove (path);
